...
- Added update() method to game/game.py
- Pieces now fall automatically over time using frame_counter and FALL_INTERVAL
- Syntax verified: python -m py_compile game/game.py passed
- No collision detection yet — piece falls through the floor
- Gemma struggled with append-only devlog and edit-without-replace — devlog now managed manually
- Learned: make Gemma show full file in chat before writing, then approve as single overwrite
